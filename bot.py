...
def handle_start(message):
    bot.send_message(message.chat.id,"приветствую я говоряший телеграмм бот пример моих возможностей в следуюшем собшении чтобы использовать мои возможности /tts если есть вопросы /help /stt если аудио")
    with open('tts.ogg', 'rb') as voice:
        bot.send_voice(message.chat.id, voice.read())

@bot.message_handler(commands=['help'])
def handle_help(message):
    bot.send_message(message.chat.id,f"Приветствую вас я телеграмм бот для озвучивания текста я оладаю лимитами 200 символов на сообшение и 1000 на пользователя чтобы начать озвучку /tts")
